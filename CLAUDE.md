...
| `.pages.yml` | Web-Editor Pages CMS (app.pagescms.org). `settings.content.merge: true` nie entfernen (sonst verschwinden Menü & Co. beim Speichern). Nach Änderungen `node scripts/editor-rundlauf.mjs` (baut alles nach einem simulierten Speichern jeder Seite und vergleicht). Siehe `docs/WARTUNG.md` 2d. |
